...
def generate(selected_tab, keys, *values):
    ...
    if args["setting_background"]:
        temp = io.BytesIO()
        args["setting_background"].save(temp, "png")
        qrcode.to_artistic(target=out, background=temp, kind='png', scale=args["setting_scale"], border=args["setting_border"], dark=args["setting_dark"], light=args["setting_light"])
    else:
        ...
        with gr.Row():
            with gr.Column():
                ...
                with gr.Accordion("Settings", open=False):
                    ...
                    inputs["setting_background"] = gr.Image(label="Background", type="pil")
